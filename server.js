...
.then((accounts) => {
  ...
  .then((result) => {
    ...
    return deployedContract.methods.totalVotesFor(asciiToHex('Yes')).call();
  })
  .then((votesYes) => {
    console.log('votesYes', votesYes);
    return deployedContract.methods.voteForCandidate(asciiToHex('Yes')).send();
  })
  .then((voteResult) => {
